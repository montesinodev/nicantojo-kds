...
export default function RestaurantMenuPage() {
  ...
  async function toggleRestaurantOpen() {
    ...
    await supabase.from('restaurants').update({ is_open: !restaurant.is_open }).eq('id', id);
        ...
        <Card className="bg-slate-900 border-slate-800 text-white mb-6">
          ...
          <CardContent>
            <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {formError && <p className="md:col-span-2 text-red-400 text-sm">{formError}</p>}
      ...
      {Object.keys(groupedItems).length === 0 ? (
        ...
        <div className="space-y-8">
          {Object.entries(groupedItems).map(([category, categoryItems]) => (
            <div key={category}>
              <h2 className="text-lg font-bold text-slate-300 mb-3">{category}</h2>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <Button
                          ...
                          className={item.is_available
                            ? 'bg-green-900 hover:bg-green-800 text-green-300 text-xs'
